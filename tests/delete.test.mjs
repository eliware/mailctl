...
describe("delete command", () => {
  test("previews deletion without a database write", async () => {
    ...
    await expect(
      deleteMessages(["message-1"], { "dry-run": true, json: true }, db),
    ).resolves.toBeUndefined();
  });
  test("requires confirmation", async () => {
    await expect(deleteMessages(["message-1"], {}, {})).rejects.toThrow(
      "refusing to delete",
    );
  });
  test("supports query dry-runs", async () => {
    ...
    await expect(
      deleteMessages([], { query: "subject", "dry-run": true, json: true }, db),
    ).resolves.toBeUndefined();
  });
  test("requires IDs when a query returns no messages", async () => {
    const db = { query: async () => [[]] };
    await expect(deleteMessages([], { query: "missing" }, db)).rejects.toThrow(
  ...
  test("deletes records transactionally", async () => {
    ...
    await expect(
      deleteMessages(["1", "2"], { yes: true, json: true }, db),
    ).resolves.toBeUndefined();
